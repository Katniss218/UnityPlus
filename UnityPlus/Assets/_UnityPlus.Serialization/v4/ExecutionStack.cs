...
namespace UnityPlus.Serialization
{
    ...
    public class ExecutionStack
    {
        ...
        public SerializationCursor Pop()
        {
            ...
            if( cursor.WriteBackOnPop && cursor.Tracker.Parent != null )
            {
        ...
        /// </summary>
        public SerializationCursor Discard()
        {
            if( _stack.Count == 0 ) throw new InvalidOperationException( "Stack is empty" );
        ...
        {
            object currentTarget = child.Tracker.Target;
            object parentTarget = child.Tracker.Parent;
            IMemberInfo member = child.Tracker.Member;
            ...
            if( !ReferenceEquals( parentTarget, child.Tracker.Parent ) )
            {
                ...
                UpdateParentTargetInStack( child.Tracker.Parent, parentTarget );
            }
        }
        ...
        private void UpdateParentTargetInStack( object oldTarget, object newTarget )
        {
            ...
            for( int i = _stack.Count - 1; i >= 0; i-- )
            {
                ...
                if( ReferenceEquals( cursor.Tracker.Target, oldTarget ) )
                {
                    cursor.Tracker = cursor.Tracker.WithTarget( newTarget );
                    _stack[i] = cursor;
